...
const TEST_HOME = path.join(os.tmpdir(), 'ai-phone-test-' + Date.now());
process.env.HOME = TEST_HOME;

test('config module', async (t) => {
  await t.test('getConfigDir returns ~/.ai-phone', () => {
    const dir = getConfigDir();
    assert.strictEqual(dir, path.join(TEST_HOME, '.ai-phone'));
  });

  await t.test('getConfigPath returns ~/.ai-phone/config.json', () => {
    const configPath = getConfigPath();
    assert.strictEqual(configPath, path.join(TEST_HOME, '.ai-phone', 'config.json'));
  });
