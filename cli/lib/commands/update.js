 ...
 * @param {string} projectRoot - Root directory of ai-phone
...
async function fetchLatestRelease() {
  try {
    const response = await fetch(
      'https://api.github.com/repos/networkchuck/ai-phone/releases/latest',
      {
        headers: {
          'User-Agent': 'ai-phone-cli',
...
async function updateViaGit(projectRoot) {
  ...
  try {
    ...
    console.log(chalk.gray('Run "ai-phone status" to verify services\n'));
  } catch (error) {
...
function showManualInstructions(release) {
  ...
  console.log(chalk.bold('   ai-phone stop\n'));

  console.log(chalk.gray('2. Backup your configuration:'));
  console.log(chalk.bold('   cp ~/.ai-phone/config.json ~/config.json.backup\n'));

  console.log(chalk.gray('3. Run the installer:'));
  console.log(chalk.bold('   curl -sSL https://raw.githubusercontent.com/theNetworkChuck/ai-phone/main/install.sh | bash\n'));

  console.log(chalk.gray('4. Start services:'));
  console.log(chalk.bold('   ai-phone start\n'));

  console.log(chalk.yellow('⚠️  Your configuration will be preserved automatically\n'));
}

/**
 * Update command - Update AI Phone to latest version
 * @returns {Promise<void>}
 */
export async function updateCommand() {
  ...
  if (configExists()) {
    ...
    const backupPath = `${process.env.HOME}/.ai-phone/config.json.pre-update`;
  ...
  if (isGitRepo(projectRoot)) {
    ...
    try {
      ...
      console.log(chalk.gray('Visit https://github.com/networkchuck/ai-phone for manual update\n'));
    }
  }
}
