...
/**
 * Prompt for deployment mode (Admin vs Device-Only)
 * @returns {Promise<string>} Selected deployment mode
 */
async function promptDeploymentMode() {
  const { mode } = await inquirer.prompt([{
    ...
    choices: [
      {
        name: 'Admin Node (Trinity + FreePBX Provisioning)',
        value: 'admin',
        short: 'Admin'
      },
      {
        name: 'Device-Only (Individual Crew Member)',
        value: 'device',
        short: 'Device-Only'
      }
    ]
  ...
  console.log(chalk.cyan(`\nYou selected: ${mode === 'admin' ? 'Admin Node' : 'Device-Only'}\n`));

  return mode;
}

/**
 * Prompt for crew member selection
 * @returns {Promise<object>} Selected crew member config
 */
async function promptCrewMember() {
  const { crewIndex } = await inquirer.prompt([{
    type: 'list',
    name: 'crewIndex',
    message: 'Select crew member to deploy:',
    choices: CREW_MEMBERS.map((crew, index) => ({
      name: `${crew.name} (${crew.extension})`,
      value: index
    }))
  }]);

  return CREW_MEMBERS[crewIndex];
}
