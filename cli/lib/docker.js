...
export function generateEnvFile(config) {
  ...
  const lines = [
    ...
    // SIP port for Contact header (5070 when an SBC/Proxy is present, 5060 otherwise)
    ...
    '',
    '# Remote Voicemail Mount',
    `SSH_PASS=${config.voicemail?.sshPass || ''}`,
    `VOICEMAIL_MOUNT_USER=${config.voicemail?.mountUser || 'root'}`,
    '',
    ...
    '',
    '# n8n Integration',
    `N8N_WEBHOOK_URL=${config.api.n8n?.webhookUrl || ''}`,
    '',
    '# ElevenLabs TTS',
