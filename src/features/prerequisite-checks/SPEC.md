...
## Overview
Add intelligent prerequisite checking at the start of `ai-phone setup` that detects missing or outdated dependencies and offers to automatically install/upgrade them with user consent.

## User Story
As a user installing ai-phone, I want the setup to check all prerequisites BEFORE asking me configuration questions, so I don't waste time on setup only to have it fail due to missing dependencies.
...
- [ ] Log full error to ~/.ai-phone/prereq-install.log
...
### AC17: Skip Option
- [ ] `ai-phone setup --skip-prereqs` bypasses all checks
...
After installing, run 'ai-phone setup' again.
```
...
- Log all install commands to `~/.ai-phone/prereq-install.log`
- Store pre-install state in `~/.ai-phone/prereq-state.json`
