...
Transform AI Phone from a multi-component, manual-setup project into a single-command installable CLI tool. Users install via `curl | bash`, run an interactive setup wizard to configure API keys and 3CX credentials, then start everything with `ai-phone start`. Compatible with Mac and Linux.

## Problem Statement

Currently, AI Phone requires:
1. Cloning the repo
...
**Primary:** Developers and homelabbers comfortable with CLI tools and Docker, but wanting a streamlined setup experience. Technical enough to configure 3CX, but shouldn't need to understand AI Phone's internals.

## User Stories

1. **As a developer**, I want to install AI Phone with a single command, so I can get started quickly without cloning repos or managing dependencies.
...
4. **As a user**, I want to start and stop AI Phone with simple commands, so I don't have to manage multiple processes manually.
...
6. **As a user**, I want to update AI Phone easily when new versions are released.
...
- [x] **AC-1**: Running `curl -fsSL https://install.ai-phone.com | bash` (or similar) downloads and installs the `ai-phone` CLI globally
...
- [x] **AC-5**: After install, `ai-phone` command is available in PATH

### Setup Wizard

- [x] **AC-6**: `ai-phone setup` launches interactive wizard
...
- [x] **AC-13**: `ai-phone doctor` runs full health check
...
- [x] **AC-19**: `ai-phone start` launches all required services (voice-app container, gemini-api-server)
- [x] **AC-20**: Start command shows status of each component coming up
- [x] **AC-21**: `ai-phone stop` cleanly shuts down all services
- [x] **AC-22**: `ai-phone status` shows running state of all components
- [x] **AC-23**: `ai-phone logs` tails combined logs from all services

### Device Management

- [x] **AC-24**: `ai-phone device add` launches interactive device wizard
- [x] **AC-25**: Device wizard prompts for: name, SIP extension, auth credentials, ElevenLabs voice ID, system prompt
- [x] **AC-26**: `ai-phone device list` shows all configured devices
- [x] **AC-27**: `ai-phone device remove <name>` removes a device configuration

### Updates

- [x] **AC-28**: `ai-phone update` downloads and installs latest version
- [x] **AC-29**: Update preserves existing configuration

### Help

- [x] **AC-30**: `ai-phone` with no args shows help with all available commands
...
1. **Install URL**: GitHub raw URL - `curl -fsSL https://raw.githubusercontent.com/theNetworkChuck/ai-phone/main/install.sh | bash`
2. **Config location**: `~/.ai-phone/` - dedicated directory, easy to find and backup
3. **Process management**: Simple background process with PID file at `~/.ai-phone/server.pid` - minimal dependencies

## Success Metrics

- User can go from zero to working AI Phone in under 10 minutes
- Setup wizard catches 90%+ of configuration errors before first call attempt
- Single `ai-phone start` successfully launches all components
